...
Item {
  ...
  function renderRichText(raw) {
    ...
    var md0 = Markdown.normalizeMarkdownForDisplay(md, false)
    return '<div style="color: ' + fg + ';">' + Markdown.markdownLiteToHtml(md0, styleOpts) + "</div>"
  }
  ...
  function extractUrlsCached(raw) {
    ...
    root._cachedUrls = Markdown.extractUrls(raw)
    return root._cachedUrls
  }

  function openUrl(url) {
    var u = Markdown.sanitizeUrlToken(url)
    if (!u)
      return
